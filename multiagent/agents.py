"""
initial prompt -> loop: {
    Prompt Generator Agent -> [any number of intermmediate agents...] -> Consolidator Agent -> Completeness Agent
}
...
class Agent:
    ...
    def run_api(self, input_prompt):
        ...
        response = run_gpt(text_prompt, agent_role)
        return response
...
returns:
{
    ...
    "intermmediate agents": [Agent, Agent, ...],
    "Consolidator Agent": Consolidator Agent,
    "Completeness Agent": Completeness Agent
}
"""
def initialize_prompt_agents(list_of_intermmediate_agents):
    prompt_generator_agent = Agent(agent_definitions["Prompt Generator Agent"]["name"], agent_definitions["Prompt Generator Agent"]["role"], agent_definitions["Prompt Generator Agent"]["function"])
    consolidator_agent = Agent(agent_definitions["Consolidator Agent"]["name"], agent_definitions["Prompt Generator Agent"]["role"], agent_definitions["Prompt Generator Agent"]["function"])
    completeness_agent = Agent(agent_definitions["Completeness Agent"]["name"], agent_definitions["Prompt Generator Agent"]["role"], agent_definitions["Prompt Generator Agent"]["function"])

    intermmediate_agents = []
    for name in list_of_intermmediate_agents:
        if name in agent_definitions and name != "Prompt Generator Agent" and name != "Consolidator Agent" and name != "Completeness Agent":
            new_agent = Agent(agent_definitions[name]["name"], agent_definitions[name]["role"], agent_definitions[name]["function"])
            intermmediate_agents.append(new_agent)

    agents_dict = {
        "Prompt Generator Agent": prompt_generator_agent,
        "Intermmediate Agents": intermmediate_agents,
...
if __name__ == '__main__':
    agents_dict = initialize_prompt_agents(["Clarity Agent", "monkeface", "Relevance Agent"])
...
Output:
{'Prompt Generator Agent': <__main__.Agent object at 0x7f2d3e45ffd0>, 'Intermmediate Agents': [<__main__.Agent object at 0x7f2d3bf99750>, <__main__.Agent object at 0x7f2d3bf99780>], 'Consolidator Agent': <__main__.Agent object at 0x7f2d3e663fd0>, 'Completeness Agent': <__main__.Agent object at 0x7f2d3e663ca0>}
